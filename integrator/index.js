...
app.get("/", (req, res) => res.send("Banyuwangi Marketplace Integrator Ready"));

app.get("/api/catalog", async (req, res) => {
  try {
    const [resA, resB, resC] = await Promise.all([
      axios
        .get(`${VENDOR_A}/api/produk`, axiosConfig)
        .catch(() => ({ data: [] })),
      axios
        .get(`${VENDOR_B}/api/products`, axiosConfig)
        .catch(() => ({ data: [] })),
      axios
        .get(`${VENDOR_C}/api/menu`, axiosConfig)
        .catch(() => ({ data: [] })),
    ]);
