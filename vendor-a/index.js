...
const apiKeyAuth = (req, res, next) => {
    ...
    if (!key || key !== process.env.API_SECRET) {
        return res.status(401).json({ error: "Unauthorized", message: "Akses Ditolak: API Key Salah/Tidak Ada" });
    }
    next();
};
app.use(apiKeyAuth);
...
app.post('/api/produk', async (req, res) => {
    try {
        ...
        if(e.code === 11000) return res.status(400).json({message: "Kode Produk Duplikat!"});
...
app.put('/api/produk/:kd', async (req, res) => {
    const updated = await ProductA.findOneAndUpdate({ kd_produk: req.params.kd }, req.body, { new: true });
